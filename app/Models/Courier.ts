 ...
 * A courier.
 *
 * Copied from the framework rather than published with `buddy publish model`,
 * because this app's scaffolded `storage/framework/defaults` tree predates the
 * Driver -> Courier rename (stacksjs/stacks#2382) and still ships `Driver`.
 * Owning it here also means the app's own models define its whole schema, which
 * is what `buddy migrate` builds from.
 */
